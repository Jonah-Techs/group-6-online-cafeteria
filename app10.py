...
def index():
    ...
    def order_approval():
        # Retrieve selected item keys from the form
        order_keys = request.form.getlist('order')
        
        # Build order details and calculate total price
        order_details = []
        total_price = 0
        for key in order_keys:
            if key in menu:
        ...
        # Render the Order Approval page and pass along the data, including order_keys
...
def submit_order():
    # Get order data from hidden fields submitted from the Order Approval page
    order_keys = request.form.getlist('order')
    order_details = []
    total_price = 0
    for key in order_keys:
    ...
    # Create and store the order (here we simply add it to an in-memory list)
    new_order = {
        'order_id': len(orders) + 1,
        'customer_name': customer_name,
        'location': location,
    ...
    # Render a final confirmation page (or you could redirect elsewhere)
    return render_template('confirmation.html',
                           customer_name=customer_name,
                           location=location,
                           order=order_details,
